...
def einlesen_ecosia(schlagwort):
    ...
    for i in final_list:
        ...
        if "momentan noch keinen Text und du bist auch nicht dazu berechtigt, diese Seite zu erstellen." in soup2.get_text() or "Diese Seite existiert nicht" in soup2.get_text() or "zur Unterscheidung mehrerer mit demselben Wort bezeichneter Begriffe" in soup2.get_text():
            ...
            break
    #Vorgang abschließen
    assoziationsliste1.append(maxT)
    return maxT

def einlesen_spiegel(schlagwort):
    url = 'https://www.spiegel.de/suche/?suchbegriff=' + schlagwort
    word_list = []
    source_code = requests.get(url)
    soup = BeautifulSoup(source_code.text, "html.parser")
    ...
    for word in split_list:
        if not word.istitle():
            word = ""
        symbols = "!:.,;‚+'<>&/()=?\"#[]" # "-" hier besser weggelassen
        forbiddenwords = ["Kurzform", "Kurzwort", "Abkürzung", "Wörter", "Plural", "Singular", "Homo", "Name", "Jahr", "Bei", "Beim", "Jahrhundert", "Prozent", "Zeichen", "Symbol", "Wort", "Bezeichnung", "Sache", "Sprache", "Logik", "Gliederung", "Definition", "Bedeutung", "Dieser", "Duden", "Begriff", "Beispiel", "Leerzeichen", "Text", "Texte", "Diese", "Artikel", "Siehe", "Wiki", "Der", "Die", "Das", "Bearbeiten", "Seite", "Seiten", "Zufällige", "Letzte", "Änderungen", "Änderung", "Sie", "Er", "Es", "Ein", "Eine", "In", "Im"]
        for i in range(0, len(symbols)):
            word = word.replace(symbols[i], "")
        for i in forbiddenwords:
            if word == i:
                 word = ""
        for i in assoziationsliste2:
            if word == i:
                word = word.replace(i, "")
        if schlagwort in word:
            word = ""
    ...
    for i in final_list:
        ...
        if "momentan noch keinen Text und du bist auch nicht dazu berechtigt, diese Seite zu erstellen." in soup2.get_text() or "Diese Seite existiert nicht" in soup2.get_text() or "zur Unterscheidung mehrerer mit demselben Wort bezeichneter Begriffe" in soup2.get_text():
            ...
            break
    #Vorgang abschließen
    assoziationsliste2.append(maxT)
    return maxT


def einlesen_wikipedia(schlagwort):
    url = 'https://de.wikipedia.org/wiki/' + schlagwort
    word_list = []
    source_code = requests.get(url)
    soup = BeautifulSoup(source_code.text, "html.parser")
    for post_text in soup.find_all('p'):
    ...
    for i in final_list:
        ...
        if "momentan noch keinen Text und du bist auch nicht dazu berechtigt, diese Seite zu erstellen." in soup2.get_text() or "Diese Seite existiert nicht" in soup2.get_text() or "zur Unterscheidung mehrerer mit demselben Wort bezeichneter Begriffe" in soup2.get_text():
            ...
            break
    #Vorgang abschließen
    assoziationsliste3.append(maxT)
    return maxT
...
while True:
    try:
        wert = input("Geben Sie ein Suchwort ein, zu dem drei verschiedene" +
        " Assoziationsliste erstellt werden soll. Falls Sie die Zahl 1 eingeben, wird das Programm beendet.")
        if wert == "1":
            break
        assoziationsliste1 = []
        assoziationsliste2 = []
        assoziationsliste3 = []
        assoziationsliste1.append(wert)
        assoziationsliste2.append(wert)
        assoziationsliste3.append(wert)
        wert1 = wert
        wert2 = wert
        wert3 = wert
        white = "                    "
        wikix = "Wiki:" + white
        ecosiax = "Ecosia:" + white
        spiegelx = "Spiegel:" + white
        print(wikix[:20], ecosiax[:20], spiegelx[:20])
        for i in range(anzahl):
            wert1 = einlesen_wikipedia(wert1)
            wert1x = wert1 + white
            wert2 = einlesen_ecosia(wert2)
            wert2x = wert2 + white
            wert3 = einlesen_spiegel(wert3)
            wert3x = wert3 + white
            print(wert1x[:20], wert2x[:20], wert3x[:20])
    except:
        print()
        print("Es ist ein Fehler aufgetreten. Wir entschuldigen uns für Unannehmlichkeiten.")
